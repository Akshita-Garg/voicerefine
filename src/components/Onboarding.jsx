...
const PROVIDERS = [
  ...
  { value: 'ollama', label: 'Local Ollama',                       needsKey: false, description: 'Free, runs on your machine. Requires running the app at localhost.', hostedOnly: true },
  ...
  return (
    <div className="flex flex-col items-center gap-8 w-full max-w-xl">
      ...
      <div className="flex flex-col gap-2 w-full">
        {PROVIDERS.map(({ value, label, description, hostedOnly }) => {
          const selected  = provider === value
          const isDisabled = hostedOnly && !isLocalhost
          return (
            <button
              key={value}
              onClick={() => !isDisabled && handleProviderChange(value)}
              disabled={isDisabled}
              className={`text-left px-4 py-3 rounded-xl border transition-all duration-150 ${
                isDisabled
                  ? 'opacity-40 cursor-not-allowed bg-[#E6CFC7] border-[rgba(58,47,42,0.08)]'
                  : selected
                    ? 'bg-[rgba(127,175,143,0.12)] border-[#7FAF8F]/50'
            ...
            >
              ...
              <p className="text-xs text-[#8A766E] leading-snug">
                {isDisabled
                  ? <>Only works when running the app at <code className="font-mono">localhost</code>. <a href="https://github.com/Akshita-Garg/voicerefine" className="underline hover:text-[#3A2F2A]">Run the app locally</a> to use Ollama.</>
                  : description}
              </p>
            </button>
          )
